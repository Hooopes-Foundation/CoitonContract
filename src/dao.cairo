...
#[starknet::contract]
mod dao {
    use coiton::interfaces::{
        idao::{User, Listing, RealestateIndexData, IDao, IDaoDispatcher, IDaoDispatcherTrait},
        ierc20::{IERC20Dispatcher, IERC20DispatcherTrait},
        icoiton_erc721::{ICoiton721Dispatcher, ICoiton721DispatcherTrait}
    };
    use super::{IERC1155EXTDispatcher, IERC1155EXTDispatcherTrait};
    use starknet::{
    ...
    use core::{
        poseidon::PoseidonTrait, hash::{HashStateTrait, HashStateExTrait}, num::traits::Zero,
        integer::u256
    };
    ...
    impl DaoImpl of IDao<ContractState> {
        ...
        fn approve_dao_member(ref self: ContractState, address: ContractAddress) {
            ...
            let erc1155_dispatcher = IERC1155EXTDispatcher {
                contract_address: self.erc1155_address.read()
            };
        ...
        fn store_realestate_index(ref self: ContractState, indices: Array<RealestateIndexData>) {
            ...
            for data in indices {
                ...
                self
                    .realestate_index
                    .write(index, RealestateIndexData { timestamp: get_block_timestamp(), ..data });
            };
        ...
        fn register_user(
            ref self: ContractState, is_dao: bool, region: ByteArray, details: ByteArray
        ) {
        ...
        fn create_listing(
            ref self: ContractState, region: ByteArray, details: ByteArray, hash: felt252
        ) {
        ...
        fn approve_listing(ref self: ContractState, _id: u256, hash: felt252) {
            ...
            let erc20_dispatcher = IERC20Dispatcher { contract_address: self.erc20_address.read() };
            let erc721_dispatcher = ICoiton721Dispatcher {
            ...
            erc721_dispatcher.mint_coiton_nft(listing.owner);
            self.listings.write(id, listing);

            self
                .unapproved_listings
                .write(
                    _id, Listing { hash: '', id: 0, owner: Zero::zero(), details: "", region: "" }
                );
        }
        ...
        fn stake_listing_fee(ref self: ContractState) {
            ...
            let erc20_dispatcher = IERC20Dispatcher { contract_address: self.erc20_address.read() };
            let allowance: u256 = erc20_dispatcher
                .allowance(get_caller_address(), get_contract_address())
                .into();
            assert(allowance >= staking_fee.into(), 'NO_ALLOWANCE');
            erc20_dispatcher
                .transfer_from(get_caller_address(), get_contract_address(), staking_fee.into());
        ...
        fn get_realestate_indices_by_region(
            self: @ContractState, region: ByteArray
